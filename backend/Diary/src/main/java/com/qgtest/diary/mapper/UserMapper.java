...
public interface UserMapper {
    ...
    //会用到修改的参数是用户名，密码，座右铭和头像（手机号和邮箱不确定先把参数整上去再说）
    @Update("UPDATE user SET username = #{username},password = #{password},motto = #{motto},avatar = #{avatar},email = #{email},phone = #{phone} ,update_time = NOW() WHERE id = #{id}")
